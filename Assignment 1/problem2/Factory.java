...
public class Factory {
    public static void main(String[] args) {
        ...
        if(bottle1<0||bottle2<0){
            System.out.println("WRONG INPUT!! No. of bottles can't be less than 0");
            return;
        }if(inp_time<0){
            System.out.println("WRONG INPUT!! Time can't be less than 0");
            return;
        }
        Time time = new Time();
        // sealing buffer tray
        ...
        while (time.currentTime <= inp_time) {
            ...
            // if current time is equal to wake up time of both sealing unit and packing
            // unit
            if (currentTime == wakeUpTimePack && currentTime == wakeUpTimeSeal) {
                // create packing unit object
                PackingUnit packing = new PackingUnit(unfinishedTray, sealUnitBuffer, pub, unfinishedSem, packingSem,
                        sealingSem, godownSem, time, godown, bottles);
                // create sealing unit object
                SealingUnit sealing = new SealingUnit(unfinishedTray, sealUnitBuffer, pub, unfinishedSem, packingSem,
                        sealingSem, godownSem, time, godown, bottles);

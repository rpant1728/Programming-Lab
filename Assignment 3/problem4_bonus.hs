...
validate_array (x:xs) currList = if ((x >= 0) && (x <=9) && ((x > 0 && not (elem x currList)) || (x == 0)))
...
sudoku puzzle = do
    if (validate_input puzzle)
      then do
        let solution = solve (array ((0, 0), (8, 8)) $ puzzleAssign puzzle)
...
possibleSymbolCombinations (row, col) board = [symbol | symbol <- [1..9], checkBoard symbol (row, col) board]
...
-- Finds solutions replacing each empty location with numbers from 1-9 recursively
...
checkRow m b (row, col) = notElem m (rowSymbols b row 8)
checkColumn m b (row, col) = notElem m (columnSymbols b col 8)                          
...
squareLocations (row, col) = [(row, col), (row, col+1), (row, col+2),
...
squareSymbols b (row, col) = [b ! loc | loc <- squareLocations (row - (row `mod` 3), col - (col `mod` 3))]

-- Assign values to a list of rows of symbols
puzzleAssign :: [[Int]] -> [((Int, Int), Int)]
puzzleAssign = concatMap rowAssign . zip [0..8]

rowAssign :: (Int, [Int]) -> [((Int, Int), Int)]
rowAssign (row, symbols) = colAssign row $ zip [0..8] symbols
...
printSudoku (Just b) = mapM_ putStrLn [show $ rowSymbols b row 8 | row <- [0..8]]